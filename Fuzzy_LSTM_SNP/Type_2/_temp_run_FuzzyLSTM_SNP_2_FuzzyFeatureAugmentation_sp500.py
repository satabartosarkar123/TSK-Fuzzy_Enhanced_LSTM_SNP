...
def gaussian_mf(x, center):
    """Fixed Gaussian membership function."""
    return np.exp(-(x - center)**2 / (2 * CURRENT_SIGMA**2))
...
# ============================================================
# Multi-Sigma Experiment Protocol
...
for CURRENT_SIGMA in SIGMA_VALUES:
    ...
    for run in range(30):
        ...
        print(f'Run {run+1} — RMSE: {rmse:.6f}, MSE: {mse:.6f}, NMSE: {nmse:.10f}, sMAPE: {s_mape:.6f}')
    ...
    sigma_results[CURRENT_SIGMA] = {
        ...
        'mean_smape': float(np.mean(all_smape)),
        'std_smape': float(np.std(all_smape)),
...
plt.title('Fuzzy Feature Augmentation — S&P 500\nPredictions vs Actual (Best of 1 run)')
